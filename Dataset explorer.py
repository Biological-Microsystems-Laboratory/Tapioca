...
def init_coco(dataset_folder, image_paths, categories, coco_json_path):
    ...
    for i, image_name in enumerate(image_paths):
        im = cv2.imread(str(image_name))
        coco_json["images"].append(
            {
                "id": i,
                "file_name": image_name.stem,
                "width": im.shape[1],
                "height": im.shape[0],
            }
        )
...
class DatasetExplorer:
    def __init__(self, dataset_folder, images_folder ):
        self.dataset_folder = dataset_folder
        self.image_paths = list(images_folder.glob("*.tif"))
        self.image_names = [obj.with_suffix("") for obj in list(images_folder.glob("*.tif"))]
        self.coco_json_path = coco_json_path
        self.categories = ["Droplet"]
        if not os.path.exists(coco_json_path):
            init_coco(
                self.dataset_folder, self.image_paths, self.categories, self.coco_json_path
            )
...
def dataset():
    dataset_result = Path("Results/dataset")
    images = Path("DropletPNGS")
    dataset_result.mkdir(exist_ok=True)
    DatasetExplorer(dataset_result, images)
    return
